...
def process_file_based_on_extension(file_content, file_extension):
    ...
    #     return extract_zip_contents(file_content)
    else:
...
def ask_openai(question, processed_content, metadata=None):
    try:
        if processed_content:
            if metadata:
                prompt = f"Attached File : {processed_content}\n Question: {question}\nAnnotator Metadata: {json.dumps(metadata)}"
            else:
                prompt = f"Attached File : {processed_content} \n Question: {question}"
        else:
            if metadata:
    ...
    if st.button("Submit"):
        if selected_question:
            with st.spinner("Waiting for OpenAI response..."):
                ...
                insert_or_update_metadata(task_id, task_level, '1' if final_answer in openai_response.lower() else '0', '0')
                if openai_response:
    ...
    if st.button("Try Again"):
        if selected_question:
            with st.spinner("Waiting for OpenAI response with annotator metadata..."):
                openai_response_with_metadata = ask_openai(selected_question, processed_content, metadata=annotator_metadata)
                insert_or_update_metadata(task_id, task_level, '-1', '1' if final_answer in openai_response_with_metadata.lower() else '0')
